...
repos_sync() {
    ...
    repo init --depth=1 -u https://github.com/rducks/android -b Q
    git clone --depth=1 https://github.com/rducks/rom.git rdx -q

    mkdir -p .repo/local_manifests
    mv rdx/q/qassa.xml .repo/local_manifests/roomservice.xml
...
build() {    
    ...
    lunch qassa_RMX2185-user
    mka qassa -j$(nproc --all) 2>&1 | tee build.log
    build_status=${PIPESTATUS[0]}   
    if [ $build_status -eq 0 ]; then
        msgtl -t "Build completed successfully"
        telegram-upload out/target/product/RMX2185/*_Sisu-*.zip --caption "Build Date: $(date)" --to "$IDTL"
        msgtl -f "build.log"
    else
